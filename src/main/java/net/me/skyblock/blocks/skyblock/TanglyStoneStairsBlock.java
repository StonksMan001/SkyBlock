...
public class TanglyStoneStairsBlock extends ModStairsBlock {
    private final Block convertsTo;
    @Nullable
    private Block driesTo;
    private final Item item;
    public TanglyStoneStairsBlock(BlockState baseBlockState, Settings settings, Item item, Block convertsTo) {
        super(baseBlockState, settings);
        this.item = item;
        this.convertsTo = convertsTo;
    }
    public TanglyStoneStairsBlock(BlockState baseBlockState, Settings settings, Item item, Block convertsTo, @Nullable Block driesTo) {
        super(baseBlockState, settings);
        this.item = item;
        this.convertsTo = convertsTo;
    ...
    protected void randomTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        if (!world.isClient && driesTo != null) {
            ...
            for (Direction direction : Direction.values()) {
                if (world.getFluidState(pos.offset(direction)).isIn(FluidTags.WATER) || state.get(WATERLOGGED)) {
                    wet = true;
                }
            }
            if (!wet) world.setBlockState(pos, driesTo.getStateWithProperties(state));
        }
    }
    ...
    protected ItemActionResult onUseWithItem(ItemStack stack, BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        ...
        if (itemStack.isOf(Items.SHEARS)) {
            ...
            world.setBlockState(pos, convertsTo.getStateWithProperties(state));
