...
public class BlueberryBush extends LeavesBlock {
    ...
    public void randomTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        ...
        if ((random.nextInt(20) == 0) && !state.get(HAS_BERRIES) && bl1) {
            if (world.getGameRules().getBoolean(SkyBlockRegistries.GameRuleRegistries.SKYBLOCK__DO_BLUEBERRIES_GROW)) world.setBlockState(pos, state.with(HAS_BERRIES, true));
        }
    }
    ...
    protected ItemActionResult onUseWithItem(ItemStack stack, BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        ...
        if (!bl) {
            ...
            dropStack(world, pos, new ItemStack(SkyBlockRegistries.ItemRegistries.SKYBLOCK__BLUEBERRIES, j + 1));
