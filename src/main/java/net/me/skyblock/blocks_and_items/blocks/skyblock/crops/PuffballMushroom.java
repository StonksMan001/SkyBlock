...
public class PuffballMushroom extends BaseShortPlantBlock {
    ...
    public void randomTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        int soil_quality = world.getGameRules().getInt(ModGameRules.SKYBLOCK__RARE_MUSHROOM_GROWTH_CHANCE);
        ...
        else if (state1.isOf(Blocks.COARSE_DIRT) || state1.isIn(ModTags.Blocks.SKYBLOCK__SPORE_PLANTER_BOXES)) soil_quality = (int)((float)soil_quality / 2f);
        ...
        if (i <= this.getMaxAge()) {
            if (random.nextInt(soil_quality) == 0) {
                if (i == this.getMaxAge()) {
                    if (world.getBlockState(pos.down()).isIn(ModTags.Blocks.SKYBLOCK__SPORE_PLANTER_BOXES)) world.setBlockState(pos, ModBlocks.SKYBLOCK__PUFFBALL_MUSHROOM_BLOCK.getDefaultState().with(PuffballMushroomBlock.NATURAL, false), 2);
                    else world.setBlockState(pos, ModBlocks.SKYBLOCK__PUFFBALL_MUSHROOM_BLOCK.getDefaultState().with(PuffballMushroomBlock.NATURAL, true), 2);
                }
