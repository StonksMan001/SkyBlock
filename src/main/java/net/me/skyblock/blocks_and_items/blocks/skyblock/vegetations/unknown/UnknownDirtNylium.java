...
public class UnknownDirtNylium extends UnknownInfectionFamily {
    ...
    public void randomTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        if (!canSurvive(state, world, pos)) {
            world.setBlockState(pos, ModBlocks.SKYBLOCK__UNKNOWN_DIRT.getDefaultState());
        }
        if (random.nextDouble() < growthChance/2 && world.getBlockState(pos.up()).isIn(BlockTags.REPLACEABLE)) {
            ...
            if (random_int == 1) {
                world.setBlockState(pos.up(), ModBlocks.SKYBLOCK__UNKNOWN_VEGETATION.getDefaultState());
            }
            if (random_int > 1) {
                world.setBlockState(pos.up(), ModBlocks.SKYBLOCK__UNKNOWN_SPROUTS.getDefaultState());
            }
        }
