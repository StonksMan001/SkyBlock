...
public class IronSkinItem extends McdArtifactItem {
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        if (!world.isClient) {
            if (Boolean.TRUE.equals(user.getStackInHand(hand).get(SkyBlockRegistries.DataComponentTypeRegistries.MCD__IRON_HIDE_AMULET_PERSONAL_TOGGLE))) {
            ...
            } else if (Boolean.FALSE.equals(user.getStackInHand(hand).get(SkyBlockRegistries.DataComponentTypeRegistries.MCD__IRON_HIDE_AMULET_PERSONAL_TOGGLE))) {
                user.getItemCooldownManager().set(this, 500);
                world.playSoundFromEntity(null, user, SkyBlockRegistries.SoundRegistries.MCD__IRON_HIDE_AMULET_USE, SoundCategory.PLAYERS, 1.0F, 1.0F);
                user.getStackInHand(hand).damage(1, user, LivingEntity.getSlotForHand(hand));
                int j = distance;
